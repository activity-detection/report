...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=3.0cm and 2.0cm, font=\sffamily]
        ...
        \node (frontend) [service, right=of nginx] {\textbf{Frontend} \\ \small \texttt{Next.js}};
        
        \node (backend) [service, below=of nginx, yshift=-0.3cm] {\textbf{Backend} \\ \small \texttt{Spring Boot}};
        
        \node (detector) [service, left=of backend] {\textbf{Detektor} \\ \small \texttt{Python Services}};
...
Przedstawiona na \myref[rysunku]{fig:architektura_systemu} architektura systemu opiera się o następujące kontenery \texttt{Dockera}: 
\begin{itemize}
...
\item \textbf{Backend} --- serwis napisany w \texttt{Spring Boot}, który obsługuje logikę biznesową, zarządza komunikacją z bazą danych oraz integruje się z modułem detekcji.
\item \textbf{Detektor} --- moduł detekcji zachowań osób, napisany w Pythonie, który przetwarza strumienie wideo w czasie rzeczywistym, analizuje je pod kątem wykrywania określonych obiektów i zachowań oraz generuje zdarzenia do dalszego przetwarzania.
...
Kod źródłowy projektu został udostępniony w repozytorium \texttt{GitHub}: 
\begin{quote}
    \href{https://github.com/activity-detection/application}{\texttt{activity-detection/application}}
\end{quote}
Organizacja \texttt{acitivity-detection} zawiera osobne repozytoria na każdy z modułów systemu, jednak wraz z rozwojem projektu zdecydowano się przenieść je do jednego repozytorium. O ile wspólne repozytorium jest optymalnym rozwiązaniem, o tyle \texttt{github subtree} pozwala na dalsze utrzymywanie modułów w osobnych repozytoriach lub cykliczną ich synchronizację z repozytorium nadrzędnym.
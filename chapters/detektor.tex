...
\subsection{Ogólna skuteczność i ograniczenia detekcji}
Podsumowując obserwacje z testów, przez większość czasu modele poprawnie wykrywają obiekty obecne w kadrze. Najwyższą skuteczność osiąga moduł anonimizacji, czyli wykrywanie twarzy oraz tablic rejestracyjnych; jedynie sporadycznie trafia się pojedyncza klatka, na której twarz nie zostaje zamazana. Największym ograniczeniem pozostaje wielkość obiektów oraz ich wzajemne sąsiedztwo: drobne lub odległe obiekty bywają pomijane, a chwilowe zasłanianie postaci przez inne osoby lub elementy sceny stanowi główną bolączkę detekcji. Problem ten można łagodzić zwiększaniem rozdzielczości obrazu kierowanego do modelu, co jednak, jak wspomniano, odbywa się kosztem wydajności.

Przy obecnej konfiguracji zdarzają się zatem pewne braki w detekcji, nie są one jednak krytyczne, o ile od systemu nie wymaga się stuprocentowej wykrywalności. Charakterystyczne jest przy tym, że w detekcji obiektów częściej dochodzi do pominięcia obiektu niż do błędnego wykrycia, to znaczy przeważają braki nad fałszywymi trafieniami. Podobnie zachowuje się rozpoznawanie zachowań: jest ono wystarczająco skuteczne, a pomyłki polegają raczej na braku wykrycia niż na wskazaniu nieistniejącego zdarzenia. Warto zaznaczyć, że wykrycie zachowania następuje często dopiero w jego trakcie, na przykład upadek rozpoznawany jest, gdy osoba już leży, co wynika z natury klasyfikatora LSTM operującego na oknie czasowym. Jest to jednak kompensowane przez bufor klatek poprzedzających zdarzenie: skoro w momencie wykrycia wiadomo już, że zdarzenie trwa, do nagrania dołączana jest odpowiednio większa liczba klatek sprzed tej chwili, dzięki czemu zapisany klip obejmuje całe zdarzenie wraz z jego rzeczywistym kontekstem.

\section{System zarządzania wykrywanymi zachowaniami}

\subsection{Skrócony opis}
System zarządzania wykrywanymi zachowaniami stanowi główny element 
zarządzający danymi w projekcie. Komponent ten został zaimplementowany w języku Java z 
wykorzystaniem frameworka Spring Boot, co pozwoliło na uzyskanie wydajnego przetwarzania, 
skalowalności oraz modułowości.

Głównym przeznaczeniem serwisu jest pełnienie roli centralnego punktu integracyjnego całego systemu. 
...
punkty końcówe wykorzystywane przez interfejs użytkownika. Serwis odpowiada za 
...
\subsection{Infrastruktura projektowa}

\subsubsection{Model składowania danych}
...
\begin{itemize}
    \item \textbf{Relacyjna baza danych (PostgreSQL)} - Odpowiada za składowanie ustrukturyzowanych danych mapowań przechowywanych nagrań oraz szablonów detekcji. W bazie danych przechowywane są również przekazywane przy zapisie nagrań wykrytych w nich elementów, z odpowiednimi znacznikami czasowymi.
    \item \textbf{Lokalny system plików (Pamięć dyskowa)} - Stanowi fizyczny magazyn dla zapisywanych nagrań. Dane na dysku kategoryzowane wewnątrz struktury katalogów, których nazwy odpowiadają dacie rejestracji, a poszczególne pliki wideo posiadają w nazwie precyzyjny znacznik czasu ich zapisu, co umożliwia administratorom i operatorom, ich ręczną lokalizację bezpośrednio z poziomu systemu operacyjnego, w razie gdyby zaszła taka potrzeba.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \caption{Stan bazy danych z zamapowanymi nagraniami.}
        \label{mapowania}
    \end{subfigure}
    ...
    \label{przyklady_mapowanych_danych}
\end{figure}


\subsubsection{Architektura potoku zapisu i mapowania danych}
System zarządzania wykrywanymi zachowaniami w ramach zapisu i odczytu działa według zdefiniowanego potoku. 
Po wyemitowaniu przez detektor zestawu danych detekcji (pliku wideo wraz z opisem jego detekcji w formacie JSON), 
...
i budowę powiązanego dokumentu JSON zawierającego szczegóły detekcji.
...
serwis zapisuje w bazie danych wiersz mapujący zapisany plik wideo w bazie PostgreSQL. 
...
\begin{figure}[H]
    ...
    \label{fig:potok_zapis_odczyt}
\end{figure}


\subsection{Opis funkcjonalności}

\subsubsection{Logika sekwencjonowania nagrań wideo}
...
\begin{itemize}
    ...
    \item 
\end{itemize}
Zastosowanie wspólnego \texttt{origin\_id} eliminuje konieczność iteracyjnego, kosztownego przeszukiwania bazy „element po elemencie” podczas odczytu, kierując się jedynie dowiązaniami z \texttt{referenced\_video\_id}, przy wyszukiwaniu sekwencji. Zamiast tego system jednym zapytaniem SQL pobiera wszystkie rekordy o danym \texttt{origin\_id}, a dopiero następnie w pamięci RAM układane są one w prawidłowej kolejności przy wykorzystaniu \texttt{referenced\_video\_id}, przygotowując do wyświetlenia.
...
\begin{figure}[H]
    ...
    \label{fig:diagram_sekwencji}
\end{figure}

\subsubsection{Mechanizm naprawy powiązań sekwencji}
System, poza usuwaniem całych sekwencji, pozwala z poziomu API na kasowanie pojedynczych, 
...
elementami sekwencji. W jego ramach realizujący są trzy scenariusze:
\begin{enumerate}
...
\subsubsection{Serwis synchronizujący mapowania danych}
...
\subsubsection{Zarządzanie szablonami detekcji}
Podsystem udostępnia pełną funkcjonalność zarządzania szablonami reguł za pośrednictwem API 
i powiązanego interfejsu webowego operatora. Użytkownik ma możliwość tworzenia, edycji, pobierania 
oraz usuwania konfiguracji szablonów. Każdy szablon posiada unikalną nazwę oraz tablicę 
...
\subsubsection{Rejestracja i zapis nagrań detekcji}
Zapis fragmentów wideo oraz powiązanych z nimi opisów wykrytych elementów, otrzymanych 
bezpośrednio z modułu detektora, odbywa się za pośrednictwem punktu końcowego REST API. 
...
danych w ramach jednego zapytania HTTP. 
...
a następnie waliduje budowę powiązanego dokumentu JSON zawierającego opis detekcji. 
Następnie plik wideo jest fizycznie zapisywany na dysku twardym w odpowiednio odatowanym katalogu, 
a do jego nazwy dopisywany jest precyzyjny znacznik czasowy. Dopiero po otrzymaniu potwierdzenia 
o prawidłowym zapisie pliku na dysku, serwis zapisuje w bazie PostgreSQL wiersz mapujący. 
...
\begin{itemize}
    \item \texttt{file} - Parametr zwierający surowe nagranie wideo z zarejestrowanym incydentem detekcji. Akceptowane są pliki w formacie MP4 oraz MOV. Plik ten po przejściu walidacji formatu jest zapisywany w strukturze dyskowej serwera.
    ...
    \item \texttt{relative-path} - Ścieżka relatywna, która instruuje serwis, w jakim konkretnie miejscu wewnątrz odatowanego katalogu nadrzędnego ma zostać zapisany plik wideo na dysku lokalnym. Umożliwia to elastyczny zapis wideo mimo zastosowania automatycznie datowanych katalogów.
    ...
    \item \texttt{details} - Blok w formacie JSON, generowany moduł detektora. Zawiera on ustrukturyzowany opis detekcji, w tym wykaz klas wszystkich wykrytych obiektów oraz znaczniki czasowe określające zakres wykrycia elementów wewnątrz przesyłanego wideo.
\end{itemize}
...
\begin{figure}[H]
    ...
    \label{przykladowe_zapytanie_zapisu_detekcji}
\end{figure}

\subsubsection{Strumieniowanie multimediów przy użyciu protokołu DASH}
Ostatnią kluczową funkcjonalnością jest dostarczanie wideo do interfejsu użytkownika obsługujące 
podzielone na fragmenty nagrania detekcji. System wykorzystuje do tego standard DASH 
(\textit{Dynamic Adaptive Streaming over HTTP}), opierając przesył danych o mechanizm częściowej 
zawartości (\textit{Partial Content}) przy wykorzystaniu kodu odpowiedzi HTTP 206. 

Na bazie logicznie ułożonej sekwencji, serwis generuje w locie manifest XML z rozszerzeniem 
\texttt{.mpd} (\textit{Media Presentation Description}). Dokument ten informuje odtwarzacz 
przeglądarki o osadzeniu czasowym poszczególnych fragmentów oraz adresach URL, 
...
fizycznego podzielenia danych na dysku, odtwarzany materiał zachowuje iluzcję ciągłości nagrania,
przy jedcznoczesnym wyeliminowaniu opóźnień sieciowych i długiego buforowanie, jak w przypadku jednolitych
